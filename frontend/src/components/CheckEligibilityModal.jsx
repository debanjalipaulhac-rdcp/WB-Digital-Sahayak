...
export default function CheckEligibilityModal({ isOpen, onClose, schemeId }) {
    ...
    function handleClose() {
        dispatch({ type: 'RESET' }); setResult(null); setChecking(false); onClose()
    }
    ...
    async function handleNext() {
        ...
        setChecking(true)
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/check-eligibility`, {
                method: 'POST',
            ...
            setResult({
                ...
                band: 'GREEN',
                schemes: [{ name: 'Sabuj Sathi', slug: 'sabuj-sathi' }, { name: 'SVMCM', slug: 'svmcm' }],
            })
        }
        setChecking(false)
    }
                                    ...
                                    <div style={{ textAlign: 'left', marginBottom: 20 }}>
                                        {(result.schemes || []).map(s => (
                                            <div key={s.slug} style={{
                                                ...
                                                <a href={`/scheme/${s.slug}`} onClick={handleClose} style={{ color: '#1A56DB', fontSize: 13, textDecoration: 'none', fontWeight: 500 }}>
                                                    View →
                                                </a>
                                            </div>
                                        ))}
